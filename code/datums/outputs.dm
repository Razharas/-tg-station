...
/datum/outputs/proc/send_info_new(mob/receiver, turf/turf_source, vol as num, vary, frequency, falloff, channel = 0, pressure_affected = TRUE)
	var/sound/sound_output
	//Pick sound
	if(sounds.len)
		var/soundin = pickweight(sounds)
...
//				return //No sound
	...
	var/image/new_image = image('icons/effects/blood.dmi', ,"itemblood", HUD_LAYER)
	var/matrix/m = matrix()
	m.Scale(rand(1,1000) / 400)
	m.Turn(rand(1,359))
	new_image.transform = m

	receiver.display_output(sound_output, new_image, text, turf_source, vol, vary, frequency, falloff, channel, pressure_affected)
